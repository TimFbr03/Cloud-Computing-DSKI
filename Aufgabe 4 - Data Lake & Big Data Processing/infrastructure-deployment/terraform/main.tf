...
# Fügen Sie nach der bestehenden Konfiguration hinzu:

# Zusätzliche Worker Nodes für Hadoop (optional)
resource "openstack_compute_instance_v2" "hadoop_worker" {
  count       = 1  # Zusätzlicher Node für Hadoop
  name        = "hadoop-${random_id.cluster_id.hex}-worker-${count.index + 1}"
  image_id    = "f445d5f0-e9a6-4e09-b3c4-7e6607aea9fb"
  flavor_name = "mb1.large"
  key_pair    = "tfa_pub_key"
...
[hadoop_worker]
%{ for hw in openstack_compute_instance_v2.hadoop_worker ~}
${hw.network.0.fixed_ip_v4} ansible_user=ubuntu ansible_ssh_private_key_file=~/.ssh/id_rsa
%{ endfor ~}
